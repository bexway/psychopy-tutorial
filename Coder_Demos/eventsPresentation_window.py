...
win = visual.Window([400,400], units='pix', monitor='testMonitor', color=[74, 196, 237], 
	colorSpace="rgb255")
...
grass = visual.Rect(win,lineWidth=0,fillColor=[30, 229, 15], fillColorSpace="rgb255",
	size=[2,.3],pos=[0,-.45], units="height")
...
for trial in parametersreader:
	...
	#Response screen
	background = visual.Rect(win,lineWidth=0,fillColor="black",size=[400,400],pos=[0,0], 
	units="height")
	#Cover up the background with black; the square should be the same size as the window
	responseText = visual.TextStim(win,text='Was that cool?', height=40, color='white',pos=[0,0])
	promptText = visual.TextStim(win,text='Press f for yes and j for no',
		height=20, color='white',pos=[0,-40])
	background.draw()
	responseText.draw()
	promptText.draw()
